...
const DYNAMIC_CACHE_NAME = 'taskflow-dynamic-v2';
...
self.addEventListener('push', event => {
  ...
  let data = { title: 'TaskFlow', body: 'Новое уведомление', reminderId: null };
  if (event.data) {
  ...
  const options = {
    ...
    vibrate: [200, 100, 200],
    tag:     data.reminderId ? `reminder-${data.reminderId}` : 'taskflow-task',
...
self.addEventListener('notificationclick', event => {
  ...
  console.log('[SW] Клик по уведомлению, action:', action);
  ...
  if (action === 'snooze') {
    ...
    event.waitUntil(
      fetch(`/snooze?reminderId=${reminderId}`, { method: 'POST' })
        .then(res => {
          console.log('[SW] Snooze ответ:', res.status);
          notification.close();
        })
        .catch(err => {
          console.error('[SW] Snooze ошибка:', err);
          notification.close();
        })
    );
  } else {
